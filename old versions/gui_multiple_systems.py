...
class App(ctk.CTk):
    ...
    def build_multiple_system_tab(self):
        ...
        for col, (text, cmd) in enumerate(buttons):
            btn_width = 180 if col == 0 else 145
            btn = ctk.CTkButton(
            toolbar,
            text=text,
            width=btn_width,              
    ...
    def calculate_multiple(self):
        def thread_fn():
            try:
                ...
                self.expansions_max_default = [math.ceil(total_points_sum / 32),math.ceil(total_points_sum / 18),math.ceil(total_points_sum / 4),math.ceil(total_points_sum / 4)]
                ...
                print("Using expansions max:", expansions_max)
