...
class System:
    ...
    def _required_total_points(self):
        """Return total required points for the system (sum of all numeric entries)."""
        return int(sum(v for v in self.system_points.values() if isinstance(v, (int, float))))

    def find_combinations(self):
        total_combinations = []

        # === AUTO-SELECT expansions_max based on total required points ===
        required_total_points = self._required_total_points()
        # Order must match expansions_list order: [XM90, XM70, XM30, XM32]
        print(self.expansions)
        self.expansions_max = [
            math.ceil(required_total_points / 32),
            math.ceil(required_total_points / 18),
            math.ceil(required_total_points / 4),
            math.ceil(required_total_points / 4),
        ]
        print("Using expansions max:", self.expansions_max)
